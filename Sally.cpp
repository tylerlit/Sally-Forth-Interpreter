   ...
   istrm(input_stream)  // use member initializer to bind reference
{
   ...
   symtab["!="]   =  SymTabEntry(KEYWORD,0,&doBangEqual) ;
   ...
   symtab["!"]    =  SymTabEntry(KEYWORD,0,&doBang) ;
...
void Sally::doLess(Sally *Sptr) {
   ...
   p2 = Sptr->params.top() ;
   Sptr->params.pop() ;
   bool answer = p2.m_value < p1.m_value ;
   if(answer){
       Sptr->params.push( Token(STRING, 1, "true") );
   }
   else{
       Sptr->params.push( Token(STRING, 0, "false") );
   }
}
...
void Sally::doBangEqual(Sally *Sptr) {
    Token p1, p2 ;
...
void Sally::doSet(Sally *Sptr){
    ...
    p2 = Sptr->params.top() ;
    Sptr->params.pop() ;
    bool exists = Sptr->symtab.find(p1.m_text) != Sptr->symtab.end() ;
    if(exists){
        throw out_of_range("Variable already exists.") ;
    } else {
        SymTabEntry data = SymTabEntry(VARIABLE, p2.m_value, NULL) ;
        Sptr->symtab.insert(pair<string, SymTabEntry>(p1.m_text, data)) ;
    }
}
...
void Sally::doBang(Sally *Sptr) {
    ...
    bool exists = Sptr->symtab.find(p1.m_text) != Sptr->symtab.end() ;
    if(exists){
        Sptr->symtab.at(p1.m_text).m_value = p2.m_value ; 
    }
    else{
        throw out_of_range("Variable does not exist.") ;
    }
